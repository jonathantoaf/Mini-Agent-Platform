...
The interesting parts: tenant isolation at both the application layer *and* PostgreSQL Row-Level Security, a mock LLM that produces fully reproducible multi-turn conversations, and 100% test coverage including dedicated RLS integration tests.
...
| **Service** | Business logic, orchestration, domain exceptions. Transport-agnostic -could serve gRPC tomorrow. | `services/` |
...
This is the most interesting part of the system. The run pipeline orchestrates prompt construction, LLM interaction, tool execution, and history storage in a controlled loop with safety guardrails.

```mermaid
sequenceDiagram
    Client->>RunService: POST /agents/{id}/run
    RunService->>RunService: Validate model + guardrail check
    RunService->>DB: Load agent + tools

    loop Until final response or max iterations
        RunService->>MockLLM: complete(messages, tools)
        MockLLM-->>RunService: tool call or final response
        RunService->>ToolExecutor: execute(tool_name, args)
        ToolExecutor-->>RunService: result
    end

    RunService->>DB: Store execution
    RunService-->>Client: RunResponse
```

### Pipeline Components

| Component | What it does | Why it's designed this way |
|-----------|-------------|--------------------------|
...
| **PromptBuilder** | Builds structured prompts: system instructions, tool descriptions, user input | Clean separation between prompt sections matters for LLM reliability |
...
Rather than inventing a custom message format, all data models (`ChatMessage`, `ToolCallData`, `FunctionCall`, `ToolDefinition`) follow OpenAI's chat completions shape. The mock LLM's `complete(messages, tools)` mirrors the real API contract. When the time comes to plug in OpenAI or Anthropic, the entire pipeline -prompt builder, execution loop, history storage -requires zero structural changes. Only the adapter implementation swaps out. This is the [Adapter Pattern](https://refactoring.guru/design-patterns/adapter) applied to LLM integration.

### 4. Domain Exceptions, Not HTTP Exceptions in Services

FastAPI makes it tempting to raise `HTTPException` directly from services. I didn't -services raise domain exceptions like `AgentNotFoundError` and `PromptInjectionError`, and routers catch those to map to HTTP status codes. The service layer is completely transport-agnostic. You could wire it to a gRPC interface, a CLI, or a background worker without touching a single line of business logic.
...
Python convention is `snake_case`, but frontend and mobile consumers expect `camelCase` in JSON. `SharedBaseModel` uses `pyhumps` as a Pydantic alias generator to handle the translation automatically. A small detail, but it shows awareness of who's actually calling the API.

### 8. Structured Logging with Request Correlation

In a multi-tenant system, "an error occurred" is useless without context. Every log line carries a `request_id` (8-char hex) and `tenant_id` via Python `contextvars`, which propagate automatically through async call chains. No explicit parameter threading needed. It's the difference between "we had an error" and "tenant_2's request `abc123ef` failed at the guardrail check." This is the same pattern Stripe and Datadog use.

### 9. Two Database Roles (Superuser for DDL, App User for Runtime)

PostgreSQL superusers bypass RLS entirely. If the app connects as `postgres`, the RLS policies are decorative -they do nothing. Migrations run as `postgres` (needs DDL privileges), but the app connects as `app_user` (non-superuser) so RLS is actually enforced at runtime. It means managing two connection strings (`DATABASE_URL` vs `DATABASE_MIGRATION_URL`), but there's no other way to make RLS real.
...
The inner loop: edit code, run `check-fast` (takes seconds), then `test` when you're confident, then commit.
...
All application tests are unit tests with mocked dependencies. Services and routers are tested in isolation using `AsyncMock` and FastAPI's `dependency_overrides`. The one exception: `test_rls.py`, which hits a real PostgreSQL instance to verify Row-Level Security actually works at the database level.
